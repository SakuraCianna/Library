...
当前仓库处于第一阶段工程骨架：已经完成桌面壳、中文三栏界面、Rust 命令边界、本地 SQLite 元数据骨架、本地 LanceDB 向量库骨架、真实文件夹登记、文件元数据扫描入库、普通文档和 XLSX 工作表结构洞察，以及本地备份导出的最短链路。
...
当前普通 PDF 解析是 MVP 轻量能力，适合可抽取文本的 PDF。扫描版 PDF 和图片可以排队 OCR，再在解析队列中启动后台 OCR worker，并通过队列刷新查看阶段、页/段进度、错误和取消状态；队列错误会在界面上隐藏本机绝对路径并限制长度。工具栏的“导出备份”会把当前知识库的本地数据库元数据写入应用数据目录下的 `backups` 子目录，当前只做导出，不做导入或恢复。复杂版面、复杂表格推理、备份恢复和 OCR 结果版面还原仍是后续能力。
...
- `Set-Location .\app\src-tauri; cargo test`：87 个 Rust 测试通过
