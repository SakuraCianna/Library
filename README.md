...
扫描会识别 `.md`、`.txt`、`.docx`、`.xlsx`、`.pdf` 和常见图片文件。新增或变更的普通文档会自动放入 `document` 后台解析队列；图片文件不会进入普通文档解析队列，可通过文件列表里的 OCR 操作进入本地 OCR 队列。点击建索引/摘要会启动文档解析 worker，将可抽取文本转成统一知识块并写入本地 SQLite。助手回答会先检索本地知识块；如果本机配置了 DeepSeek API Key，会尝试使用 `deepseek-v4-flash` 生成更自然的回答，失败时自动回退到本地检索结果。
...
当前扫描阶段会在后台任务中遍历支持格式文件并计算内容指纹，扫描完成后自动为新增、变更和失败的普通文档创建文档解析任务；图片文件保留在文件列表中，等待用户按需排队 OCR。超大目录扫描后续仍需要接入文件数量/大小限制和更细粒度的实时事件流。
